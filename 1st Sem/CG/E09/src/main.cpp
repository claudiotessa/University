...
// vertex format that only holds position data (for rendering roads as lines)
struct VertexLine {
	glm::vec3 pos;
};
...
class E09 : public BaseProject {
	...
	void localInit() {
		...
		VertexDescriptor VDline;
		VDline.init(this, {
				{0, sizeof(VertexLine), VK_VERTEX_INPUT_RATE_VERTEX}
			}, {
				{0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VertexLine, pos),
					sizeof(glm::vec3), POSITION}
			});

		VDRs.resize(5); // Resize to accommodate the new vertex descriptor
		VDRs[4].init("VDline", &VDline);

		Pipeline Pline;
		Pline.init(this, &VDline, "shaders/SimpleLine.vert.spv", "shaders/SimpleLine.frag.spv", {&DSLglobal});
		Pline.setPrimitiveTopology(VK_PRIMITIVE_TOPOLOGY_LINE_STRIP); // This is key!
		// You may also want to set the line width
		// Pline.setLineWidth(2.0f);
