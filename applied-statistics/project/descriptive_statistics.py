"""
Descriptive statistics on the exoplanets dataset
"""
...
df = pd.read_csv("PS_2026.03.11_03.14.31.csv", comment="#")
...
cols_to_keep = [
    ...
    "pl_orbsmax",
    "pl_bmasse",
    "pl_rade",  # Planet physics (Earth units)
    "st_teff",
    "st_mass",
    "st_rad",
    "st_met",  # Stellar demographics
    "sy_pnum",
    "sy_gaiamag",  # System stats
]
# Dropping NaNs for a cleaner initial analysis
df_sub = df[cols_to_keep].dropna()

print(f"Dataset shape: {df_sub.shape}")
print()
...
print("Statistics for numerial variables:")
print(df_sub.describe())
print()

# Count planets per star
planet_counts = df_sub["hostname"].value_counts()
print("Top 5 systems by planet count:")
print(planet_counts.head())
print()

# Descriptive stats of the distribution itself
print("Statistics for number of planets per system:")
print(planet_counts.describe())
print()

# Categorize the stars into 3 groups based on stellar mass (st_mass)
# Red Dwarfs (0 to 0.5), Sun-like (0.5 to 1.2), Massive (> 1.2)
df_sub["star_type"] = pd.cut(
    df_sub["st_mass"],
    bins=[0, 0.5, 1.2, 10],
    labels=["Red Dwarf", "Sun-like", "Massive"],
)


# Now we calculate the average Earth-Mass of planets orbiting each star type
grouped_stats = df_sub.groupby("star_type", observed=False)["pl_bmasse"].mean()

print("Average Planet Mass (in Earth Masses) by Star Type:")
print(grouped_stats)
print()

# Calculate Density relative to Earth (Earth = 1.0)
df_sub["density_earth_units"] = df_sub["pl_bmasse"] / (df_sub["pl_rade"] ** 3)

# Calculate Surface Gravity relative to Earth (Earth = 1.0)
df_sub["gravity_earth_units"] = df_sub["pl_bmasse"] / (df_sub["pl_rade"] ** 2)

print("Descriptive Statistics for Earth-Similarity Features:")
print(df_sub[["density_earth_units", "gravity_earth_units"]].describe())
print()
