...
2. **Distance between consecutive iterates**. Define $\boldsymbol{\delta}^{(k)} = \mathbf{x}^{(k+1)} - \mathbf{x}^{(k)}$ $$\boxed{\lVert \boldsymbol{\delta}^{(k)} \rVert \leq \varepsilon} $$It can be shown that the relation between the true error and $\boldsymbol{\delta}^{(k)}$ is$$\lVert \mathbf{e}^{(k)} \rVert \leq \frac{1}{1 }$$This is a good stopping criterion only if $\rho(B) \ll 1$.