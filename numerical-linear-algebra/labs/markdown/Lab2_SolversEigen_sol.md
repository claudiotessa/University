...
Then, load the previous rhs and compute the new one with the following script
```cpp
#include <iostream>
#include <Eigen/Sparse>
#include <unsupported/Eigen/SparseExtra>
...
int main(int argc, char** argv)
{
    ...
    loadMarketVector(b, "rhs.mtx");

    VectorXd xe = VectorXd::Constant(mat.rows(), 1);      
    // define exact solution
    VectorXd b2 = mat*xe;                 // compute right-hand side
    cout << "difference = " << (b-b2).norm() << endl;
    return 0;    
}
```
3. If we remove additional entries (e.g. smaller that 1e-9) we can se a difference:
```
sed -i '/e-1[0-9]/d' mhd416a.mtx 
wc - l mhd416a.mtx
./ex2
```
